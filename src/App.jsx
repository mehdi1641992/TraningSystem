...
// ---- Access rule ---------------------------------------------------------
// Staff / coordinators use single-word emails: name@<org domain>
// Participants use two-part emails:            firstname.lastname@<org domain>
// mehdi@<org domain> is the permanent Superadmin -- always full
...
// or plain Staff (read-only calendar, no roster needed). Participants need to be
// added to the Participant list before they can sign in at all.
...
const STAFF_EMAIL_RE = new RegExp('^[a-z]+@' + ORG_DOMAIN_ESC + '$', 'i');
const FELLOW_EMAIL_RE = new RegExp('^[a-z]+\\.[a-z]+@' + ORG_DOMAIN_ESC + '$', 'i');
const SUPERADMIN_EMAIL = 'mehdi@' + ORG_DOMAIN;
...
const DEFAULT_ACCOUNTS = [
  { email: 'mehdi@' + ORG_DOMAIN, name: 'Mehdi Morshed Chowdhury', role: 'lead' },
  { email: 'asifur@' + ORG_DOMAIN, name: 'Md Asifur Rahman', role: 'coordinator' },
  { email: 'hasibur@' + ORG_DOMAIN, name: 'Hasibur Rahman Sohan', role: 'facilitator' }
];
...
const STAFF_ADMIN_ROLES = ['lead', 'coordinator'];
const AFA_ROLES = ['facilitator'];
...
function friendlyFellowName(entry, roster) {
  ...
  return found?.name || entry.fellowName || entry.name || entry.email || String(entry.fellowId || 'Participant');
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    return onAuthStateChanged(firebaseAuth, async (user) => {
      ...
      if (!user.email?.toLowerCase().endsWith('@' + ORG_DOMAIN)) {
        await signOut(firebaseAuth);
        setAuthError('Use your ' + ORG_NAME + ' Google account to continue.');
        setAuthLoaded(true);
        return;
      }
  ...
  const handleLocalLogin = role => {
    ...
    const localUser = role === 'superadmin' ? {
      ...
      email: 'demo.fellow1@' + ORG_DOMAIN, name: 'DEMO Participant 01', role: 'fellow', fellowId: 'demo-fellow-1', afaGroup: 'DEMO Facilitator 1', uid: 'local-fellow', localTest: true
    };
...
async function resolveRole(user) {
  ...
  if (FELLOW_EMAIL_RE.test(em)) {
    ...
    return { ok: false, error: "This email isn't on the approved Participant list yet. Ask a planner to add it." };
  }

  return { ok: false, error: 'Enter a valid @' + ORG_DOMAIN + ' email -- staff use name@' + ORG_DOMAIN + ', Participants use firstname.lastname@' + ORG_DOMAIN + '.' };
}
...
function MainApp({ auth, onLogout }) {
  ...
  const persistAssessmentAttempts = debouncedPersist(setAssessmentAttempts, attemptsSaveTimer, ASSESSMENT_KEYS.attempts, err => { if (err) showToast('Save failed: ' + (err.code || err.message) + ' -- check that you are signed in with a ' + ORG_NAME + ' account.'); });
    ...
    try {
      ...
      rows.forEach((row, idx) => {
        ...
        newSessions.push({
          ...
          fellowNotes: findValue(row, ['Participant Notes', 'fellow notes', 'Participant Note', 'fellow note']),
          afaGroup: findValue(row, ['Facilitator Group', 'afa group', 'Facilitator', 'afa', 'Group', 'group']),
          calendared: true,
        });
      });
...
const pageExporters = {
  calendar: () => exportSheetFile('Participant_Training_System_Calendar.xlsx', [{ name: 'Calendar', rows: sessionRows() }]),
  sessions: () => exportSheetFile('Participant_Training_System_Sessions.xlsx', [{ name: 'Sessions', rows: sessionRows() }]),
  staffCalendar: () => exportSheetFile('Participant_Training_System_Staff_Calendar.xlsx', [{ name: 'Calendar', rows: sessionRows() }]),
  summary: () => exportSheetFile('Participant_Training_System_Time_Summary.xlsx', [{ name: 'Time Summary', rows: sessions.map(session => ({ Week: session.week, Mode: session.mode, Session: session.name, DurationMinutes: session.start && session.end ? durationMin(session) : '' })) }]),
  fellows: () => exportSheetFile('Participant_Training_System_Fellows.xlsx', [{ name: 'Participants', rows: roster.map(fellow => ({ Name: fellow.name, Email: fellow.email, Track: fellow.track, Grade: fellow.grade, PlacementCity: fellow.placementCity, AFAGroup: fellow.afaGroup, RoomIds: (fellow.roomIds || []).join(', ') })) }]),
  rooms: () => exportSheetFile('Participant_Training_System_Rooms.xlsx', [{ name: 'Rooms', rows: rooms.map(room => ({ Name: room.name, Facilitator: room.facilitator || '', LocationType: room.locationType || '', PhysicalLocation: room.physicalLocation || '', OnlinePlatform: room.onlinePlatform || '', MeetingUrl: room.meetingUrl || '', AccessInstructions: room.accessInstructions || '', FellowIds: (room.fellowIds || []).join(', ') })) }]),
  sessionTypes: () => exportSheetFile('Participant_Training_System_Session_Types.xlsx', [{ name: 'Session Types', rows: sessionTypes.map(t => ({ Name: t.name, Color: t.color || '' })) }]),
  pillarTags: () => exportSheetFile('Participant_Training_System_Pillars.xlsx', [{ name: 'Pillars', rows: pillarTags.map(p => ({ Name: p.name })) }]),
  modes: () => exportSheetFile('Participant_Training_System_Work_Modes.xlsx', [{ name: 'Work Modes', rows: modes.map(m => ({ Name: m.name })) }]),
  requests: () => exportSheetFile('Participant_Training_System_Requests.xlsx', [{ name: 'Requests', rows: requests.map(r => ({ SessionId: r.sessionId, SessionName: r.sessionName || '', RequesterEmail: r.requesterEmail || '', RequesterRole: r.requesterRole || '', Message: r.message || '', CreatedAt: r.createdAt || '', Resolved: r.resolved ? 'Yes' : 'No' })) }]),
  assessments: () => exportSheetFile('Participant_Training_System_Assessments.xlsx', [
    { name: 'Assessments', rows: assessments.map(assessment => ({ Title: assessment.title, SessionId: assessment.sessionId, Status: assessment.status, Questions: (assessment.questions || []).length, Participants: (assessment.assignmentGroups || []).flatMap(group => group.fellowIds || []).length })) },
  ...
  review: () => exportSheetFile('Participant_Training_System_Paragraph_Reviews.xlsx', [{ name: 'Paragraph reviews', rows: assessmentAttempts.flatMap(attempt => Object.entries(attempt.reviews || {}).map(([questionId, review]) => ({ AssessmentId: attempt.assessmentId, FellowId: attempt.fellowId, QuestionId: questionId, Status: review.status || '', Score: review.score ?? '', Feedback: review.feedback || '', Reviewer: review.reviewer || '', ReviewedAt: review.reviewedAt || '', AISuggestedScore: review.aiSuggestion?.suggestedScore ?? '', AIConfidence: review.aiSuggestion?.confidence ?? '', AIFeedback: review.aiSuggestion?.feedback || '' }))) }]),
  analytics: () => exportSheetFile('Participant_Training_System_Analytics.xlsx', [{ name: 'Analytics', rows: sessions.map(session => ({ Session: session.name, OnTime: attendance.filter(entry => entry.sessionId === session.id && entry.status === 'on_time').length, Late: attendance.filter(entry => entry.sessionId === session.id && entry.status === 'late').length })) }]),
  incidents: () => exportSheetFile('Participant_Training_System_Incidents.xlsx', [{ name: 'Incidents', rows: assessmentIncidents.map(item => ({ Time: item.createdAt || '', FellowId: item.fellowId || '', AssessmentId: item.assessmentId || '', Type: item.type || '', DeviceId: item.deviceId || '', Details: item.details || '' })) }]),
  devices: () => exportSheetFile('Participant_Training_System_Devices.xlsx', [{ name: 'Devices', rows: deviceRequests.map(r => ({ RequestedAt: r.requestedAt || '', FellowId: r.fellowId || '', AssessmentId: r.assessmentId || '', OldDeviceId: r.oldDeviceId || '', NewDeviceId: r.newDeviceId || '', Status: r.status || '', ResolvedAt: r.resolvedAt || '' })) }]),
  planners: () => exportSheetFile('Participant_Training_System_WA_Staff.xlsx', [{ name: 'WA Staff', rows: planners.map(p => ({ Name: p.name, Email: p.email || '', Role: p.role || '', CallSign: p.callSign || '', Group: p.group || '' })) }]),
  roles: () => exportSheetFile('Participant_Training_System_Roles.xlsx', [{ name: 'Roles', rows: roles.map(r => ({ Id: r.id, Label: r.label || '' })) }, { name: 'City codes', rows: (cityCodes || []).map(c => ({ City: c.city, Code: c.code })) }]),
};
...
const seedDemo = () => {
  if (!window.confirm('Create demo Participants, sessions, questions, and assessments?')) return;
  const demoFellows = Array.from({ length: 10 }, (_, index) => ({ id: `demo-fellow-${index + 1}`, name: `DEMO Participant ${String(index + 1).padStart(2, '0')}`, email: `demo.fellow${index + 1}@${ORG_DOMAIN}`, afaGroup: `DEMO Facilitator ${index % 3 + 1}`, track: index % 2 ? 'secondary' : 'primary', placementCity: ['Dhaka', 'Chattogram', 'Rajshahi'][index % 3] }));
  const demoSessions = Array.from({ length: 5 }, (_, index) => ({ id: 90000 + index, week: index % 3, date: `2026-11-${String(1 + index).padStart(2, '0')}`, weekday: 'Sunday', start: '10:00', end: '11:00', name: `DEMO Session ${index + 1}`, pillar: 'Academic Content', mode: 'Workshop', facilitators: ['Demo Facilitator'], resources: [], outcomes: ['Demo outcome'], notes: '', fellowNotes: '', afaGroup: `DEMO Facilitator ${index % 3 + 1}`, calendared: true }));
...
function Sidebar({ tab, setTab, isAdmin, isFullAdmin, isSuperadmin, isFellow, openRequests }) {
  ...
  const tabs = [
    ...
    ...(isFullAdmin ? [
      ...
      { id: 'fellows', label: 'Participants', icon: UsersThree },
...
function isSessionVisibleToFellow(session, auth, roster, rooms) {
  ...
  if (!hasTargeting) return true; // sessions with no room/Facilitator targeting are visible to everyone
...
function FellowAttendanceBreakdown({ sessions, attendance, fellows, roster }) {
  ...
  return <div style={{ marginTop: 24 }}>
    <div style={{ fontWeight: 700 }}>Participant-wise attendance</div>
...
function RosterPanel({ roster, staff, cityCodes, onChange, onAccount, showToast }) {
  ...
  const afaStaff = (staff || []).filter(p => p.role === 'facilitator' && p.name);
  ...
  const addOne = (e) => {
    ...
    if (!FELLOW_EMAIL_RE.test(em)) { setError('Email must look like firstname.lastname@' + ORG_DOMAIN); return; }
    ...
    setName(''); setEmail(''); setAfaGroup(''); setTrack(''); setGrade(''); setPlacementCity(''); setError(''); showToast('Participant added');
  };
  const removeOne = (id) => { onChange(roster.filter(r => r.id !== id)); showToast('Participant removed'); };
  const editOne = (fellow) => { const name = window.prompt('Participant name', fellow.name); if (!name?.trim()) return; const email = window.prompt('Participant email', fellow.email); if (!email?.trim()) return; const afa = window.prompt('Facilitator group', fellow.afaGroup || '') ?? (fellow.afaGroup || ''); const next = { ...fellow, name: name.trim(), email: email.trim().toLowerCase(), afaGroup: afa.trim(), track: (window.prompt('Track: primary or secondary', fellow.track || '') || fellow.track || '').toLowerCase(), grade: window.prompt('Grade', fellow.grade || '') || fellow.grade || '', placementCity: window.prompt('Placement city', fellow.placementCity || '') || fellow.placementCity || '', roomIds: fellow.roomIds || [] }; onChange(roster.map(item => item.id === fellow.id ? next : item)); onAccount(next, 'fellow'); showToast('Participant updated'); };
  const importBulk = () => {
...
function PlannerPanel({ planners, roles, onChange, onAccount, showToast }) {
  ...
  const addOne = (e) => {
    ...
    const em = email.trim().toLowerCase();
    if (!name.trim()) { setError('Enter a name.'); return; }
    if (!STAFF_EMAIL_RE.test(em)) { setError('Email must look like name@' + ORG_DOMAIN); return; }
    ...
    setName(''); setEmail(''); setRole('lead'); setAccess('full'); setGroup(''); setCallSign(''); callSignTouched.current = false; setError(''); showToast('Staff added');
  };
  ...
  const saveEdit = (e) => {
    ...
    if (!STAFF_EMAIL_RE.test(em)) { setEditError('Email must look like name@' + ORG_DOMAIN); return; }
    ...
    onAccount(updated, updated.role || 'lead');
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', display: 'flex', justifyContent: 'flex-end', zIndex: 100 }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ width: 400, maxWidth: '92vw', background: '#18181B', height: '100%', overflowY: 'auto', padding: 22, boxShadow: '-8px 0 24px rgba(0,0,0,.12)' }}>
        ...
        <Field label="Participant-visible notes"><textarea className={inputStyle + ' resize-y'} rows={3} value={form.fellowNotes} onChange={e => set('fellowNotes', e.target.value)} placeholder="Notes Participants should see" /></Field>
